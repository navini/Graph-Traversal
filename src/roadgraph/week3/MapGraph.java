...
public class MapGraph {
										  ...
										  GeographicPoint goal, Consumer<GeographicPoint> nodeSearched)
	{
	    ...
	    PriorityQueue<Intersection> exploringIntersections = new PriorityQueue<Intersection>(getNumVertices(), comparator);
	    ...
	    while(exploringIntersections.size() > 0) {
	    	...
	    	if(current.equals(endIntersection)) {
	    		...
	    		//Add unvisited neighbors with shortest distance from the current node to the queue, visited set and parent map
	    		for(Edge e : current.getEdges()) {
	    			if(!visitedSet.contains(e.getEnd())) {
	    				if(current.getCost()+e.getLength() < e.getEnd().getCost()) {
	    					e.getEnd().setCost(current.getCost()+e.getLength());
	    					parentMap.put(e.getEnd(), current);
	    					exploringIntersections.add(e.getEnd());
	    				}
	    			}
		...
		//Get the start and goal intersections from the map
			    ...
			    while(exploringIntersections.size() > 0) {
			    	...
			    	if(current.equals(endIntersection)) {
			    		...
			    		for(Edge e : current.getEdges()) {
			    			if(!visitedSet.contains(e.getEnd())) {
			    				Intersection neighbour = e.getEnd();
			    				double distanceToEnd = Math.hypot(goal.getX()-neighbour.getLocation().getX(), goal.getY()-neighbour.getLocation().getY());
			    				if(current.getCost()+e.getLength()+distanceToEnd < neighbour.getCost()) {
			    					e.getEnd().setCost(current.getCost()+e.getLength()+distanceToEnd);
			    					parentMap.put(neighbour, current);
			    					exploringIntersections.add(neighbour);
			    				}
			    			}
			    		}
			    	}
